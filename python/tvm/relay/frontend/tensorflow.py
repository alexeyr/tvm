...
def _reshape():
    def _impl(inputs, attr, params):
        ...
        try:
            shape_arg = _get_list_param(params, pop_node)
        except AttributeError:
            # Shape operator is already pruned, hence
            # try to infer shape by precompute prune if possible.
            func = _expr.Function(ir_pass.free_vars(pop_node), pop_node)
            ...
            m.set_input(**params)
            m.run()
            shape_arg = m.get_output(0).asnumpy().astype('int64').flatten().tolist()
        return AttrCvt(
            op_name="reshape",
