...
def verify_dense_int8(batch, in_dim, out_dim, use_bias=True):
    ...
    def get_ref_data():
        a_np = np.random.randint(low=-128, high=127, size=(batch, in_dim), dtype=dtype)
        b_np = np.random.randint(low=-128, high=127, size=(out_dim, in_dim), dtype=dtype)
        c_np = np.random.randint(low=-128, high=127, size=(out_dim,), dtype=out_dtype)
